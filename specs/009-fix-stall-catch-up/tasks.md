...
- [X] T009 [US3] Re-read `tests/lib/loop/stall.test.ts` against FR-017's checklist end to end (normal frame, sub-boundary stutter, boundary-itself, past-boundary, long-stall-equals-short-stall, zero/negative/non-finite totality) and confirm each is a distinct, separately named `it`/`it.each` case rather than folded into a single loose assertion, so a future change that widens the boundary or restores a second clamp fails a specific, legible test (SC-006, FR-017 Acceptance Scenario 4).
